...
#include <sys/stat.h>
#include "SDL.h"
#include "read_printer_interface.c"
#include "dir.c"

/* Retro-Printer Printer Emulator - Program by RWAP Software to convert the data captured by the read_printer_interface.c
 * program.  The raw file is stored as  as a raw data file and then the ESC/P2 data is used to create a PDF file
 * and output that data to a connected printer
 * v1.1a
 *
 * v1.0 First Proof of concept version - ESC/P2 only
 * v1.1 Update to include configuration files
 * v1.1a Expanded range of ESC/P codes checked for - up to page 96 of the Epson programming reference manual
 */

// Configuration options
#define EMULATION_CONFIG    "/root/config/emulation"
#define RETAIN_DATA_CONFIG  "/root/config/retain_data"
#define PRINTER_CONFIG      "/root/config/output_printer"
#define PDF_CONFIG          "/root/config/output_PDF"
#define LINEFEED_CONFIG     "/root/config/linefeed_ending"
#define PATH_CONFIG         "/root/config/output_path"
#define PRINTERDELAY_CONFIG "/root/config/printer_delay"
...
int printing = 0;           // 1=print to printer 0=do not print
int pdf = 0;                // 1=create pdf 0=do not create pdf
struct timespec tvx;
int startzeit;              // start time for time measures to avoid infinite loops
int delayprinter = 0;       // Pause between /strobe + /busy and reset of /busy.
                            // Some machines like ste need that pause 
...
// von 100 auf 0 gesetzt (21.01.2015)
...
int curtab = 0;                             // next active tabulator
...
int test_for_new_paper()
{
    ...
    if ((ypos > (2525 - 17 * vPixelWidth)) || (state == 0)) {
        ...
        if (printing == 1) {
            sprintf(filenameX, "lpr -o fit-to-page %spage%d.pdf &", pathpdf, page);
            printf("command = %s \n", filenameX);
            system(filenameX);
        }
        if (fp != NULL) {
           ...
           float xzoom, float yzoom)
{
    ...
    for (opr = 0; opr < dotColumns; opr++) {
        ...
        for (fByte = 0; fByte < 8; fByte++) {
            if (xd & 128) putpixelbig(xpos, ypos2 + fByte * dpiv / 72, 1, 1);
            xd = xd << 1;
        }
        xpos = xpos + hPixelWidth;
    }
...
int main(int argc, char *args[])
{
    ...
    if (argc < 5) {
        printf
            ("Usage: ./retroprinter <timeout> <divisor> <font> <font_direction> <sdl> <path> [print] [pdf]\n\n");

        printf("Usage: ./retroprinter 4 3 font.raw 1 sdlon /home/pi/data\n \n");
        printf
        ...
        printf("print              --> print to printer\n");
        printf("pdf                --> create pdf\n");
        printf("offlineswitch      --> evaluate offline switch\n");
        printf("endlesstext        --> only write one raw file in $path/raw/1.raw\n");
    ...
    for (i = 0; i < argc; i++) {
        ...
        if (strcmp(param, "print") == 0) {
            ...
            // get default from /root/config/output_printer
            if (cfileexists(PRINTER_CONFIG)) {
                FL=fopen(PRINTER_CONFIG, "r");
                fscanf(FL, "%s", config);
                fclose(FL);
                config = lowerCaseWord(config); // Convert to lower case
                if (strcmp(config,"yes") == 0) {
                    printing=1;
                }
            }
        }
        if (strcmp(param, "pdf") == 0) {
            pdf = 1;
        } else {
            // get default from /root/config/output_PDF
            if (cfileexists(PDF_CONFIG)) {
                FL=fopen(PDF_CONFIG, "r");
                fscanf(FL, "%s", config);
                fclose(FL);
                config = lowerCaseWord(config); // Convert to lower case
                if (strcmp(config,"yes") == 0) {
        ...
        if (strncmp(param, "delay=", 6) == 0) {
            delayprinter = atoi(&args[i][6]);
        } else if (cfileexists(EMULATION_CONFIG)) {
            // get default from /root/config/printer_delay            
            FL=fopen(PRINTERDELAY_CONFIG, "r");
            fscanf(FL, "%d", delayprinter);
            fclose(FL);
        } else {
            delayprinter=0;
        }
        ...
        if (strncmp(param, "endlesstext", strlen(param)) == 0) {
            printf ("\n\nOK, endlesstext only. Implies no pdf, no bmp, no printout, no timeout, no new page.\n\n");
            endlesstext=1;
            printing=0;
            pdf=0;
            sdlon=0;
    ...
    timeout = 0;
    if (endlesstext==0) timeout = atof(args[1]);
...
main_loop_for_printing:
            ...
            if (rawispcl == 1) {
                ...
                if (xd == (int) 27) {   // ESC v 27 v 1b
                    ...
                    case '@':    // ESC @ Initialize
                        ...
                        break;                       
                    case '.':    //Raster printing
                        // Not yet implemented
                        // Check for ESC . / ESC . 2 / ESC .3
                        break;
                ...
                case 9:    // TAB
                    if (print_controlcodes) {
                        ...
                        if (hTabulators[curtab] == 0) curtab = 0; // No more tab marks
                        if (hTabulators[0] > 0) {
                            // forward numbercount of tabulator[curtab ]characters to the right 
                            xpos = (dpih * hTabulators[curtab]) / cpi;
                            curtab++;
                        }
                    }
                    break;
                ...
                if ((xd == (int) 27) && (print_controlcodes == 0) ) {   // ESC v 27 v 1b
                    ...
                    case '*':    // ESC * m nL nH d1 d2 . . . dk print bit-image graphics.
                        ...
                        case 7:  // 144x72 dpi 9 needles
                            ...
                            _8pin_line_print(dotColumns, hPixelWidth, vPixelWidth, 1.0, 1.0);
                            break;
                        case 32:  // H-Density 60dpi V-Density 180DPI 24 Dots in one
                            // row (3bytes)
                            needles = 24;
                            hPixelWidth = (float) dpih / (float) 60;
                            ...
                            break;
                        case 33:  // mode 33dez 120x180dpi 24 needles
                            needles = 24;
                            hPixelWidth = (float) dpih / (float) 120;
                            vPixelWidth = (float) dpiv / (float) 180;  // Das sind hier 1.2 
                            // Pixels
                            _24pin_line_print(dotColumns, hPixelWidth, vPixelWidth, 1.0, 1.0);
                            break;
                        case 35:  // Resolution not verified k?te auch 240x216 sein
                            needles = 24;
                            hPixelWidth = (float) dpih / (float) 240;
                            vPixelWidth = (float) dpiv / (float) 180;  // Das sind hier 1.2 
                            // Pixels
                        ...
                        case 38:  // mode 38dez 90x180 dpi
                            ...
                            _24pin_line_print(dotColumns, hPixelWidth, vPixelWidth, 1.0, 1.0);
                            break;
                        case 39:  // mode 39dez 180x180 dpi
                            needles = 24;
                            ...
                            _24pin_line_print(dotColumns, hPixelWidth, vPixelWidth, 1.0, 1.0);
                            break;
                        case 40:  // mode 40dez 360x180 dpi
                            needles = 24;
    ...
    if (sdlon) SDL_UpdateRect(display, 0, 0, 0, 0);
    if ((i == 0) || (endlesstext==1)) goto main_loop_for_printing;
    ...
    if ((endlesstext==0) && (fp != NULL)) {
        fclose(fp);
        fp = NULL;
    }        // close text file
    if (endlesstext==0)  page++;
    if (page > 199) {
        page = dirX(pathraw); 
        reduce_pages(page, pathraw);
        page = dirX(pathbmp);
